...
def _auth(authorization: Optional[str] = Header(default=None)) -> None:
    ...
    if token and authorization != f"Bearer {token}":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid simulator token")
...
def _handle(fn):
    try:
        ...
        raise HTTPException(status_code=exc.status_code, detail={"code": exc.status_code, "message": exc.message})
...
def customers_list(
    ...
    return _ok(svc.list_customers(db, page, page_size, gender=gender, age_group=age_group, province=province, customer_type=customer_type))


@router.post("/customers", response_model=dict)
def create_customer(body: CustomerIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def update_customer(customer_id: int, body: CustomerUpdateIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_customer(customer_id: int, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_customers(body: BulkDeleteIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
    result = svc.delete_many(svc.delete_customer, db, body.ids)
    if result["deleted"]:
        _publish("customer", "delete", {"ids": result["deleted"], "count": len(result["deleted"])})
    return _ok(result)


@router.get("/products/list", response_model=dict)
def products_list(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), category: Optional[str] = None, status: Optional[str] = None, _: None = Depends(_auth), db: Session = Depends(get_db)):
    return _ok(svc.list_products(db, page, page_size, category=category, status=status))


@router.post("/products", response_model=dict)
def create_product(body: ProductIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def update_product(product_id: int, body: ProductUpdateIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_product(product_id: int, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_products(body: BulkDeleteIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
    result = svc.delete_many(svc.delete_product, db, body.ids)
    if result["deleted"]:
        _publish("product", "delete", {"ids": result["deleted"], "count": len(result["deleted"])})
    return _ok(result)


@router.get("/orders/list", response_model=dict)
def orders_list(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), status: Optional[str] = None, _: None = Depends(_auth), db: Session = Depends(get_db)):
    return _ok(svc.list_orders(db, page, page_size, status=status))


@router.post("/orders", response_model=dict)
def create_order(body: OrderIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def update_order_status(order_id: int, body: OrderStatusIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_order(order_id: int, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_orders(body: BulkDeleteIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
    result = svc.delete_many(svc.delete_order, db, body.ids)
    if result["deleted"]:
        _publish("order", "delete", {"ids": result["deleted"], "count": len(result["deleted"])})
        _publish("finance", "delete", {"order_ids": result["deleted"]})
    return _ok(result)


@router.get("/finance/list", response_model=dict)
def finance_list(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), type: Optional[str] = None, category: Optional[str] = None, _: None = Depends(_auth), db: Session = Depends(get_db)):
    return _ok(svc.list_finance(db, page, page_size, type_=type, category=category))


@router.post("/finance", response_model=dict)
def create_finance(body: FinanceIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def update_finance(finance_id: int, body: FinanceUpdateIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_finance(finance_id: int, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def notifications_list(page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), ntype: Optional[str] = None, is_read: Optional[bool] = None, _: None = Depends(_auth), db: Session = Depends(get_db)):
    return _ok(svc.list_notifications(db, page, page_size, ntype=ntype, is_read=is_read))


@router.post("/notifications", response_model=dict)
def create_notification(body: NotificationIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def update_notification(notification_id: int, body: NotificationUpdateIn, _: None = Depends(_auth), db: Session = Depends(get_db)):
...
def delete_notification(notification_id: int, _: None = Depends(_auth), db: Session = Depends(get_db)):
