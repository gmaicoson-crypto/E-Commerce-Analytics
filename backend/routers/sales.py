...
async def sales_overview(
    ...
    order_change = (
        ((order_count - order_count_prev) / order_count_prev * 100)
        if order_count_prev
        else 0
    )
    ...
    data = [
        {
            ...
            "avg_order_value": (
                float(stats["sales"] / stats["order_count"])
    ...
    for item, order in items:
        ...
        if pid not in product_stats:
            product_stats[pid] = {
                ...
                "category": (
                    item.product.category.value if item.product.category else "Unknown"
                ),
                "quantity": 0,
                "sales": Decimal(0),
            }
    ...
    data = [
        {
            ...
            "sales": float(stats["sales"]),
            "avg_price": (
                float(stats["sales"] / stats["quantity"])
                if stats["quantity"] > 0
                else 0
            ),
        }
        for pid, stats in sorted(
            product_stats.items(), key=lambda x: x[1]["sales"], reverse=True
        )[:limit]
    ]
