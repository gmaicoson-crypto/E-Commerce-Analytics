...
// color 默认 'currentColor' → SVG 继承父元素 CSS color,任何忘记传 color 的调用方都能拿到默认描边色
const props = withDefaults(defineProps<{ name: string; size?: number; color?: string }>(), { size: 16, color: 'currentColor' })
const ICONS: Record<string, string> = {
  barChart:    '<rect x="3" y="12" width="4" height="9"/><rect x="10" y="6" width="4" height="15"/><rect x="17" y="3" width="4" height="18"/>',
  ...
  refresh:     '<polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-4.95"/>',
...
const path = computed(() => ICONS[props.name] ?? ICONS['alertCircle'])
</script>
