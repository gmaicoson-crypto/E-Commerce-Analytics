<template>
  <div class="pg">
    ...
    <button class="pg-btn" :disabled="page <= 1" @click="goto(page - 1)"><AppIcon name="chevronLeft" :size="14" /></button>
    <template v-for="(p, i) in visiblePages" :key="i">
      <button v-if="typeof p === 'number'" class="pg-btn" :class="{ active: page === p }" @click="goto(p)">{{ p }}</button>
      <span v-else class="pg-dots">…</span>
    </template>
    <button class="pg-btn" :disabled="page >= totalPages" @click="goto(page + 1)"><AppIcon name="chevronRight" :size="14" /></button>
  </div>
</template>
