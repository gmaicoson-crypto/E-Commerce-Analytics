  ...
  <div class="fade-in">
    ...
    <div class="kpi-grid mb16">
      ...
      <KpiCard
        ...
        :sub-value="`¥${fmtMoneyCN(overview.pending_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
      <KpiCard
        title="已支付"
        ...
        :sub-value="`¥${fmtMoneyCN(overview.paid_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
      <KpiCard
        title="已发货"
        ...
        :sub-value="`¥${fmtMoneyCN(overview.shipped_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
      <KpiCard
        title="已完成"
        ...
        :sub-value="`¥${fmtMoneyCN(overview.completed_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
      <KpiCard
        title="已取消"
        ...
        :sub-value="`¥${fmtMoneyCN(overview.cancelled_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
      <KpiCard
        title="已退款"
        ...
        :sub-value="`¥${fmtMoneyCN(overview.refunded_amount ?? 0, { prefix: '' })}`"
        sub-label="金额"
      />
    </div>
...
import type { OrderListItem, TableColumn } from '@/types'
import {
  AXIS_GRID,
  ORDER_STATUS_EN,
  ORDER_STATUS_ZH,
...
async function loadOrdersPage(page: number, pageSize: number) {
  const status =
    filter.value === '全部' ? undefined : ORDER_STATUS_EN[filter.value]
        ...
        series: [
          {
            ...
            data: list.map(s => ({
              ...
              value: s.count,
              itemStyle: {
                color: COLOR_MAP[ORDER_STATUS_ZH[s.status] ?? s.status] ?? '#999',
              },
            })),
        ...
        yAxis: [
          {
            ...
            axisLabel: { formatter: (v: number) => `${v} 单` },
          },
          {
            type: 'value',
            ...yName('销售额 (¥)'),
        ...
        series: [
          {
            name: '订单数',
            type: 'bar',
            data: tl.map(t => t.order_count),
            itemStyle: { color: 'rgba(82,183,136,.7)', borderRadius: [3, 3, 0, 0] },
            barMaxWidth: 10,
          },
          {
            name: '销售额',
  ...
  series: [
    {
      ...
      data: statusDist.value.map(s => ({
        name: s.name,
        value: s.count,
        itemStyle: { color: COLOR_MAP[s.name] },
      })),
      label: {
        show: true,
        formatter: '{b}\n{d}%',
...
const trendOpt = computed<EChartsOption>(() => {
  ...
  return {
    ...
    xAxis: {
      ...
      axisTick: { show: false },
      splitLine: { show: false },
      axisLabel: { interval: Math.max(1, Math.floor(days / 6)) },
    },
    yAxis: {
      type: 'value',
      ...yName('订单数 (单)'),
      ...
      h(
        'span',
        {
          style: {
            ...
            fontSize: '12px',
            fontWeight: 700,
            background: BADGE_BG[v as string] || '#f1f5f0',
            color: BADGE_FG[v as string] || '#4b5563',
          },
        },
        String(v),
      ),
