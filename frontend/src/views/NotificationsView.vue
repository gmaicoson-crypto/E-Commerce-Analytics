<template>
  <div class="fade-in">
    <PageHeader title="通知中心" subtitle="系统预警与重要通知（仅管理员）">
      <template #actions>
        <AppBadge v-if="unread > 0" :label="`${unread} 条未读`" color="red" size="md" />
        <AppBtn size="sm" variant="ghost" icon="checkCircle" @click="markAllRead">全部标为已读</AppBtn>
      </template>
    </PageHeader>
    <div class="filter-row">
      <button v-for="t in types" :key="t" class="type-btn" :class="{ active: filter === t }" @click="filter = t">
        {{ t }}
      ...
      <div v-for="n in filtered" :key="n.id" class="notif-item" :style="{ borderLeftColor: n.read ? 'var(--border)' : TC[n.type], opacity: n.read ? 0.75 : 1 }">
        ...
        <div class="notif-body">
          <div class="notif-meta">
            <span class="notif-badge" :style="{ background: TB[n.type], color: TC[n.type] }">{{ n.type }}</span>
...
const TC: Record<string,string> = { '库存预警':'#ca8a04','大额退款':'#dc2626','异常订单':'#ea580c','销售额波动':'#2563eb' }
const TB: Record<string,string> = { '库存预警':'#fef9c3','大额退款':'#fee2e2','异常订单':'#ffedd5','销售额波动':'#dbeafe' }
const TI: Record<string,string> = { '库存预警':'package','大额退款':'finance','异常订单':'alertCircle','销售额波动':'sales' }
...
const filtered = computed(() => filter.value === '全部' ? notifications.value : notifications.value.filter(n => n.type === filter.value))
...
.notif-badge{ display:inline-flex;align-items:center;padding:2px 9px;border-radius:99px;font-size:12px;font-weight:700; }
.unread-dot { width:7px;height:7px;border-radius:99px;background:#ef4444;display:inline-block; }
